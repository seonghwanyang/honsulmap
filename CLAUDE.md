...
## Honsulmap-specific notes

- **Branches**: User reviews on `dev` first. Push only to `dev` unless user explicitly says "main" or "merge".
- **IG scraping cron**: lives on user's PC via Windows Task Scheduler running `python -m worker.scrape` from `worker_venv`. Render cron was retired (Cloudflare Turnstile blocked the data-center IP).
