...
// 가게 분석 (CRM) — 가게 통계 페이지의 "통계" 섹션에 인라인으로 렌더되는 대시보드.
// 이미 수집 중인 데이터만 집계(/api/partner/spots/[id]/insights). 새 수집 지표는 로드맵.
// 별도 페이지가 아니라 가게 통계 페이지 본문의 일부.

import { Fragment, useEffect, useState } from 'react';
import { Card, ACCENT } from '../../ui';
...
function StatCard({
  ...
  return (
    <Card style={{ padding: '13px 15px' }}>
      <div style={{ fontSize: 11.5, fontWeight: 700, color: SUB }}>{lab}</div>
      <div
        style={{
          fontSize: 24,
          fontWeight: 820,
      ...
      {delta !== undefined && (
        <div style={{ marginTop: 8 }}>
          <Delta v={delta} suffix={lab === '재방문율' ? '%p' : '%'} />
        </div>
      )}
    </Card>
  );
...
function BarRow({ label: lab, value, max, right }: { label: string; value: number; max: number; right: string }) {
  return (
...
export default function StoreInsights({ spotId }: { spotId: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/partner/spots/${spotId}/insights`);
        if (!res.ok) {
          const j = await res.json().catch(() => ({}));
          if (active) setErr(j.error || '불러오지 못했어요.');
          return;
        }
        const json = await res.json();
        if (active) setD(json);
      } catch {
        if (active) setErr('네트워크 오류로 불러오지 못했어요.');
      } finally {
  ...
  if (loading)
    return (
      <Card style={{ padding: 16 }}>
        <p style={{ fontSize: 12.5, color: MUTED }}>분석 불러오는 중…</p>
      </Card>
    );
  if (err || !d)
    return (
      <Card style={{ padding: '24px', textAlign: 'center' }}>
        <p style={{ color: SUB, fontSize: 13 }}>{err ?? '데이터를 불러오지 못했어요.'}</p>
      </Card>
    );
  ...
  if (!d.hasData)
    return (
      <Card style={{ padding: '40px 24px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 800, color: INK, marginTop: 10 }}>아직 분석할 데이터가 적어요</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ fontSize: 11.5, color: MUTED }}>최근 30일 · 영업 마감 집계 기준 · 손님 신원은 익명 기기키</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3" style={{ gap: 12 }}>
        <Card style={{ padding: '15px 16px' }} className="md:col-span-2">
          <div style={label}>방문 추이 <span style={{ fontSize: 11, fontWeight: 600, color: MUTED }}>· 최근 {d.daily.length}영업일</span></div>
          {d.daily.length ? (
      ...
      {/* 세그먼트 */}
      <div>
        <div style={label}>고객 세그먼트 <span style={{ fontSize: 11, fontWeight: 600, color: MUTED }}>· RFM · 최근 60일</span></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3" style={{ gap: 12 }}>
        <Card style={{ padding: '15px 16px' }}>
          <div style={label}>고객 구성 <span style={{ fontSize: 11, fontWeight: 600, color: MUTED }}>· 성별·연령</span></div>
          {genderTotal > 0 ? (
            <>
        ...
        <Card style={{ padding: '15px 16px' }}>
          <div style={label}>방문 성향 <span style={{ fontSize: 11, fontWeight: 600, color: MUTED }}>· 목적·분위기</span></div>
        ...
        <Card style={{ padding: '15px 16px' }}>
          <div style={label}>시간대 분석 <span style={{ fontSize: 11, fontWeight: 600, color: MUTED }}>· 요일×시간</span></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 12 }}>
        <Card style={{ padding: '15px 16px' }}>
          <div style={label}>메뉴 분석 <span style={{ fontSize: 11, fontWeight: 600, color: MUTED }}>· 수량·매출 TOP</span></div>
          {d.menu.length ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {d.menu.map((m) => (
        ...
        <Card style={{ padding: '15px 16px' }}>
          <div style={label}>혼술맵 유입 기여 <span style={{ fontSize: 11, fontWeight: 600, color: MUTED }}>· 방문 전→귀속</span></div>
      ...
      {/* 고객 리스트 */}
      <Card style={{ padding: '15px 16px' }}>
        <div style={label}>고객 리스트 <span style={{ fontSize: 11, fontWeight: 600, color: MUTED }}>· 최근 방문 순 · 익명 기기키</span></div>
        {d.customers.length ? (
          <div style={{ overflowX: 'auto' }}>
