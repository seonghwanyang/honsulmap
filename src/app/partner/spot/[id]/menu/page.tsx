...
// 메뉴 편집 — 카테고리/품목 직접 CRUD + 네이버 수집 메뉴 원클릭 임포트 +
// ₩0 서비스 아이템(호출·추천·신고·익명선물) 한 번에 추가.
...
function MenuEditor() {
  ...
  useEffect(() => {
    fetch(`/api/partner/spots/${id}/menu`)
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        if (!d) return;
        setSpotName(d.spot?.name ?? '');
  ...
  const save = async () => {
    setSaving(true);
    const res = await fetch(`/api/partner/spots/${id}/menu`, {
      method: 'PUT',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      <PageHeader
        title="메뉴 관리"
        subtitle={`${spotName} · 손님 QR 메뉴판에 그대로 반영돼요.`}
        action={
      ...
      {cats.length === 0 && (
        <Card dashed style={{ padding: '36px 20px', textAlign: 'center', color: '#9ca3af', fontSize: 13.5 }}>
      ...
      <button
        ...
        style={{ ...buttonStyle('outline'), alignSelf: 'flex-start' }}
      >
        <PlusIcon />
        카테고리 추가
      </button>
