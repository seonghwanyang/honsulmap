...
### 10.23 관리자 모니터 페이지 (09-15) — 플러그인·주문·스크래퍼를 한 화면에
`/admin/monitor` (dev `dbaf3f3`). 그동안 `scripts/_check_plugin_status.mjs`와 Supabase 수동 조회로 보던 것을 30초마다 자동으로 그린다. 읽기 전용, 데이터는 `/api/admin/monitor`.

| 칸 | 무엇을 보나 | 빨간불 기준 |
|---|---|---|
| 플러그인 | `plugin_last_seen` 하트비트 (폴링 중 60초에 1번 갱신) | 3분 넘게 없음 = 끊김 |
| 플러그인 버전 | 마지막 `플러그인 시작 vX.Y` 로그와 시각 | (정보) v5.3이 찍혀야 배포 적용된 것 |
| 포스 마지막 활동 | 이 매장의 마지막 토스 웹훅(주문 생성·결제·취소) | (정보) 포스가 켜져 있어야만 온다 |
| 오늘 영업분 주문 | QR 주문별 최신 ack 결과 — 테이블 반영 / 포스 직접 주문 / 폴백 / 미확인 | **ack 없이 90초 초과 = "포스에 안 들어간 주문"** 표로 나열 (좌석·금액·접수·경과) |
| 플러그인 최근 로그 | 최근 8줄 + 1시간 경고 수 + 포스 주문 보고(보류) 24h 건수 | (정보) 리밋 경고·재보고 폭주 감지용 |
| 스크래퍼 | `max(spots.last_scraped_at)` 나이 | 5분 초과 (`/api/health/scraper`와 동일) |

- 맨 위 배너에 빨간불만 모아 보여 준다. 없으면 "모두 정상".
- 09-15 실데이터 첫 화면: 플러그인 끊김 18시간, 좌석 11 11,900원 주문 46분째 미확인 — 손으로 조회하던 그 상황이 그대로 잡혔다.
- 안 하는 것: 조치 버튼(전송 모드 전환 등)은 없다. 감시 전용. 폴백 스윕이 사장님 보드 폴링에만 매달려 있는 구조(10.19)는 별도 수정.

### 10.24 관리자 페이지 렉·UX 점검 (09-15)
로컬 dev 서버에서 관리자 API 8개의 응답 시간·용량을 재고, 390px(폰) 폭으로 화면을 찍어 본 결과와 조치.

| 항목 | 전 | 후 | 조치 |
|---|---|---|---|
| `/api/admin/view-stats` (조회수) | 6.1초(웜) · 16초(콜드) | 2.9초 | 일별 집계 뷰를 1,000행씩 **순차 44왕복**하던 것 → 첫 페이지에서 총 건수를 받고 나머지 페이지를 동시 요청. 스토리 수는 분석 창(첫 게재 −7일 ~ 마지막 게재 +6일)만 읽음 |
| 대시보드 | 목록 4개를 통째로 받아 length (가게 목록만 275KB) | 80바이트 | `/api/admin/counts` 건수 전용 API |
| 가게 목록 | 755행을 한 번에 렌더 | 150행 + "N개 더 보기" | 폰 스크롤 버벅임 완화. 셀 줄바꿈 금지(가게명이 4줄로 접히던 것) |
| 상단 메뉴 (폰) | 메뉴 글자가 세로로 쪼개지고 브랜드가 3줄 | 한 줄 · 가로 스크롤 · 현재 페이지 강조 | `AdminNav` 클라이언트 컴포넌트 |
| 소비자 하단 탭 · 푸터 | 관리자 화면 위에 떠서 표·로그를 가림 | 숨김 | `BottomNav` · `Footer`의 제외 경로에 `/admin` 추가 |
| 대시보드 운영 상태 | 없음 | 플러그인 · 포스 미도달 주문 · 스크래퍼를 빨강/초록 한 줄씩 | `/api/admin/monitor` 재사용, 대시보드 맨 위 |

- 나머지 API(요청·신청·신고·공지·성과·모니터)는 0.3~1초로 문제없음.
- 조회수가 아직 2.9초인 이유: PostgREST 1,000행 한도로 26왕복이 남아서. 일별 사이트 합계 뷰(`site_views_daily`) 하나를 추가하면 1초 아래로 내려간다 — 마이그레이션이라 필요할 때 진행.
- 사장님 신청 목록은 신청 1건마다 `auth.admin.getUserById`를 부른다(N+1). 건수가 적어 지금은 체감 없음.
